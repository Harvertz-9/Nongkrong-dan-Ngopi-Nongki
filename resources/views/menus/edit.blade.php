@extends('layouts.app')
@section('title', 'Edit Menu - Nongki')
...
<section class="py-12 bg-cream min-h-[80vh]">
    <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-2xl md:text-3xl font-bold text-coffee">Edit Menu</h1>
            <p class="text-mocha text-sm mt-1">Perbarui data menu <span class="font-semibold text-coffee">{{ $menu->nama }}</span></p>
        </div>
        ...
        <div class="bg-warm-white rounded-2xl border border-latte-light/20 p-6 sm:p-8 shadow-sm">
            <form action="{{ route('menu.update', $menu->id) }}" method="POST">
                ...
                <div class="mb-5">
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3" required placeholder="Deskripsikan menu ini..." class="w-full px-4 py-3 bg-cream border border-latte-light/30 rounded-xl text-sm text-coffee placeholder-mocha/40 focus:outline-none focus:ring-2 focus:ring-latte focus:border-transparent transition resize-none @error('deskripsi') border-terracotta @enderror">{{ old('deskripsi', $menu->deskripsi) }}</textarea>
                ...
                <div class="mb-8">
                    ...
                    <div class="flex items-center gap-3">
                        <label for="tersedia" class="relative inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" id="tersedia" name="tersedia" value="1" {{ old('tersedia', $menu->tersedia) ? 'checked' : '' }} class="sr-only peer">
                        ...
                        <span class="text-sm text-coffee-light" id="tersediaLabel">{{ old('tersedia', $menu->tersedia) ? 'Tersedia' : 'Tidak Tersedia' }}</span>
                    </div>
                ...
                <div class="flex items-center gap-3">
                    ...
                    <button type="submit" class="flex-1 bg-coffee hover:bg-coffee-light text-cream font-semibold py-3 px-4 rounded-xl transition text-sm shadow-md">Perbarui Menu</button>
                </div>
            </form>
        </div>
    </div>
